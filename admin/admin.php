...
class EL_Admin {
	...
	public function show_plugin_upgrade_message() {
		if($this->show_upgr_message) {
			...
			$title = sprintf($error ? __('Errors during upgrade of plugin %1$s','event-list') : __('Upgrade of plugin %1$s successful','event-list'), '"atozsites Event"');
	...
	public function register_pages() {
		// Settings subpage
		$page = add_submenu_page('edit.php?post_type=el_events', __('atozsites Event Settings','event-list'), __('Settings','event-list'), 'manage_options', 'el_admin_settings', array(&$this, 'show_settings_page'));
		add_action('admin_print_scripts-'.$page, array(&$this, 'embed_settings_scripts'));

		// About subpage
		$page = add_submenu_page('edit.php?post_type=el_events', __('About atozsites Event','event-list'), __('About','event-list'), 'edit_posts', 'el_admin_about', array(&$this, 'show_about_page'));
